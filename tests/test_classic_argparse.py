...
def test_yaml_read_action(yaml_loader):
    ...
    parsed = parser.parse_args(
        [os.path.join(FILE_DIR, "param.yaml"), os.path.join(FILE_DIR, "param.yaml")]
    )
    assert parsed.zoo.bingo == "a"
    assert parsed.zoo.bango == 1
        ...
        == """\
{
  "object_selector": null
    ...
    parser.add_argument(
        ...
        out_stream=ss,
        parameterized=parameterized,
        only={"nesting": {"params_a": {"bingo"}, "params_b": {"list_"}}},
        indent=None,
    )
    with pytest.raises(SystemExit):
        parser.parse_args(["--print"])
    ss.seek(0)
    assert (
        ss.read().strip() == '{"nesting": {"params_b": {"list_": ["foo", "bar"]}, '
