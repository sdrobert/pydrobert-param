...
def test_combine_json_files(temp_dir):
    paths = dict((x, os.path.join(temp_dir, x + ".json")) for x in "abcde")
...
"""
    ...
    with open(out) as f:
        assert (
            f.read().strip() == '{"a": {"a": {"d": null}}, "c": 1, "d": {"foo": "bar"}}'
        )
    assert not command_line.combine_json_files(
    ...
    with open(out) as f:
        assert (
            f.read().strip() == '{"a": {"b": 1, "a": {"c": 2, "d": null}}, '
            '"c": 1, "d": {"foo": "bar"}}'
        )


def test_combine_yaml_files(temp_dir, yaml_loader):
    paths = dict((x, os.path.join(temp_dir, x + ".yaml")) for x in "abcde")
...
"""
    ...
    assert not command_line.combine_yaml_files([paths["b"], paths["c"], out, "--quiet"])
    with open(out) as f:
        assert (
            f.read().strip()
            == """\
...
- baz"""
    ...
    assert not command_line.combine_yaml_files([paths["d"], paths["e"], out, "--quiet"])
    with open(out) as f:
        assert (
            f.read().strip()
            == """\
