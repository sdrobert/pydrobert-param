...
def _serialize_to_yaml(fp, obj, help_dict=dict()):
    for name in YAML_MODULE_PRIORITIES:
        if name == "ruamel.yaml":
            ...
            raise ValueError("Invalid value in YAML_MODULE_PRIORITIES: {}".format(name))
    raise ImportError(
...
def serialize_to_json(
    ...
    `JSON syntax <https://en.wikipedia.org/wiki/JSON>`__. This function converts
    `parameterized` to a dictionary, then fills an JSON file with the contents of this
    dictionary.
    ...
    Notes
    -----
    JSON does not have standard "comments," so this function does not write help strings
    as comments
    """
    if isinstance(file, str):
        with open(file, "w") as fp:
...
def deserialize_from_yaml(
    ...
    `YAML syntax <https://en.wikipedia.org/wiki/YAML>`__. This function converts a YAML
    file to a dictionary, then populates `parameterized` with the contents of this
    dictionary
...
def deserialize_from_json(
    ...
    `JSON syntax <https://en.wikipedia.org/wiki/JSON>`__. This function converts a JSON
    file to a dictionary, then populates `parameterized` with the contents of this
    dictionary
